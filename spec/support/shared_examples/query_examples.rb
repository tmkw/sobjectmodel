RSpec.shared_examples 'defining model Query methods' do
  ...
  let(:definition) { Yamori::ClassDefinition.new(schema) }
  let(:query_condition) { instance_double('Yamori::QueryMethods::QueryCondition')  }
  ...
  describe '.select' do
    before do
      ...
      allow(Yamori::QueryMethods::QueryCondition)
        .to receive(:new)
    ...
    it 'returns a QueryCondition object' do
      ...
      expect(Yamori::QueryMethods::QueryCondition).to have_received :new
